...
LamassuConfig.prototype.isAuthorized = function (deviceId, callback) {
  this.client.query({
    text: 'SELECT * FROM devices WHERE device_id=$1 AND authorized=true',
    values: [deviceId]
...
LamassuConfig.prototype.pair = function (token, deviceId, name, callback) {
  var self = this
    ...
    if (results.rowCount === 1) {
      ...
      self.client.query({
        ...
        if (err && err.code === '23505') {
          return callback(new Error('Device with this device id already exists'))
        }
        callback(err)
      })
    } else {
...
LamassuConfig.prototype.devices = function (callback) {
  this.client.query({
    text: 'SELECT device_id, name FROM devices'
  }, function (err, results) {
    if (err) return callback(err)
...
LamassuConfig.prototype.unpair = function (deviceId, callback) {
  this.client.query({
...
LamassuConfig.prototype.authorize = function (deviceId, callback) {
  this.client.query({
    text: 'UPDATE devices SET authorized=true WHERE device_id=$1',
    values: [deviceId]
  }, callback)
}
...
LamassuConfig.prototype.authenticateUser = function (user, pwd, callback) {
  this.client.query({
    ...
    deriveKey(pwd, obj.salt, function (_err, salt, derivedKey) {
      ...
      if (obj.pwdhash === derivedKey) return callback(null, obj)
